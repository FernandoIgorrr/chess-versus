...
import 'package:chess_versus/src/data/exceptions/player_impossible_name_exception.dart';
...
class PlayerCreateUseCase {
  ...
  AsyncResult<Player> createFrom(String tournamentId, Player player) async {
    ...
    try {
      (await _tournamentRepository.findById(
        tournamentId,
      )).fold((success) => success, (failure) => throw failure);

      await _playerRepository.create(player, tournamentId);
